...
const ProductIndex = () => {
  ...
  React.useEffect(() => {
    const fetchTransactions = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${config.API_URL}/transactions?user_id=current`, {
          headers: {
            'Content-Type': 'application/json',
            'token': token || ''
          }
        ...
        if (products.length > 0 && data.transaction_items.length > 0) {
          const transactionsCountMap: Record<string, number> = {};
          
          data.transaction_items.forEach((transaction: Transaction) => {
            if (transaction.transaction_id && transaction.product_id) {
              transactionsCountMap[transaction.product_id] = (transactionsCountMap[transaction.product_id] || 0) + 1;
            }
          });
          
          const updatedProducts = products.map(product => ({
            ...product,
            transactionsCount: transactionsCountMap[product.product_id] || 0
          }));
  ...
  const handleDelete = async (productId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${config.API_URL}/products/remove/${productId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'token': token || ''
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r px-6 py-4" style={{ background: 'linear-gradient(to right, #0D6577, #0A5666)' }}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            onClick={() => navigate('/member/products/create')}
      ...
      <div className="overflow-x-auto">
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <p className="text-gray-400 mb-4">Add a new product to get started</p>
            <button
              onClick={() => navigate('/member/products/create')}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead>
              <tr>
                ...
                <th scope="col" className="min-w-[120px] px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price</th>
              ...
              {products.map((product) => (
                <tr key={product.product_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500 mt-1">{product.transactionsCount || 0} Transaction</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-teal-100 text-teal-800">
                      {product.type === 1 ? 'New' : 'Used'}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      <button
                        onClick={() => navigate(`/member/products/${product.product_id}`)}
                      ...
                      {(product.transactionsCount || 0) === 0 && (
                      <>
                      <button
                        onClick={() => navigate(`/member/products/${product.product_id}/edit`)}
