      ...
      const fetchTransaction = async () => {
        try {
          ...
          setFormData({
            ...
            shipping_image_id: data.shipping_image_id || '',
            delivered_at: data.delivered_at ? new Date(data.delivered_at).toISOString().slice(0, 16) : '',
            delivered_details: data.delivered_details || '',
            fee: data.fee || '',
            fee_type: data.fee_type
          });
  
          if (data.shipping_image_id) {
            const fileResponse = await fetch(`${config.API_URL}/files/${data.shipping_image_id}`, {
              headers: {
                'token': token || ''
              }
            });
